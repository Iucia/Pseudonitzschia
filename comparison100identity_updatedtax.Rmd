...
a) A revised version  of the **PR2 database** (Guillouet al., 2013) => **TARA-PR2** ; 
...
pwd: /home/lcampese/Data/metaB/V4/18S_V4_1191_samples.results_FIXEDTAXONOMY

wc -l: 15325278 18S_V4_1191_samples.results

srun grep "Pseudo-nitzschia" 18S_V4_1191_samples.results > 18S_V4_1191_Pn_samples.results &
  
wc -l: 41439 18S_V4_1191_Pn_samples.results
```
...
Pn_samples <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/NEWDATA/18S_V4_1191_samples.results/18S_V4_1191_Pn_samples.results", header = F, sep = "\t")

colnames(Pn_samples) <- c("seqId", "tot.numb.reads", "pid", "lineage", "ID_ref")

Percid <- Pn_samples %>%  
  select(pid) %>% 
  summary()

mainplot <- Pn_samples %>% ggplot + 
  aes(x = pid, 
      fill = tot.numb.reads) + 
  geom_histogram(bins = 100) + 
  theme_bw() + 
  annotation_custom(gridExtra::tableGrob(Percid), xmin=55, xmax=65, ymin=10000, ymax=16000) +
  ggtitle("Percentage of identity to the PR2 reference", 
          subtitle= paste0("Pseudonitzschia, TARA V4; ", 
                           n_distinct(Pn_samples$seqId), 
                           " distinct amplicons")) +
  annotate("rect", xmin=95, xmax=100, ymin=0, ymax=19500, fill="lightblue", alpha = 0.5)
...
mainplot + annotation_custom(ggplotGrob(zoomplot),
                             xmin = 70, xmax = 92, 
                             ymin = 10000, ymax = 20000)
```


The identity percentage ranges from 56.6 to 100, but almost all sequences show a value around 99% identity to the reference database.
...
srun awk '$2>2' 18S_V4_1191_Pn_samples.results > 18S_V4_1191_Pn_samples_3readsmin.results &
  
wc -l: 25383 18S_V4_1191_Pn_samples_3readsmin.results
wc -l: 41439 18S_V4_1191_Pn_samples.results

#[*]as adviced by Nicholas Henry. Note: he also suggests to take reads appearing in at least 2 stations,
#but this will be done after
#(when integrating the distribution info)
  
  
#Take only 100% pid sequences (pid is 3rd column)
awk '$3>99.99' 18S_V4_1191_Pn_samples_3readsmin.results > 18S_V4_1191_Pn_samples_3readsmin_pid100.results

#they are 7

#Extract only seq ids (column 1)
awk '{print $1}' 18S_V4_1191_Pn_samples_3readsmin_pid100.results > 18S_V4_1191_Pn_samples_3readsmin_pid100_seqid.txt

#use the retrieved 7 sequence Ids to subset the distr file (i.e. the one 
#containing seqId|sampleId|readcounts

srun grep -Ff 18S_V4_1191_Pn_samples_3readsmin_pid100_seqid.txt ~/Data/metaB/V4/18S_V4_1191_samples.distr > 18S_V4_1191_Pn_samples_3readsmin_pid100.distr &
  
 
#the final output is made by 7 different sequences

```

There are only **7** different **sequences** assigned to *Pseudonitzschia* species with a **100% identity** to PR2 database and occurring at least **3 times** in the dataset. 

<br>

3. Add the environmental information for the selected 7 sequences

Note: All sequences have length >380 bp: this is sequence length and not query cover. However, the taxonomic assignment has been done using vsearch with the usearch_global option. It is global alignment so the query cover is always 100%.

```{r warning=FALSE, message=FALSE, cache=TRUE}
library(data.table)
library(tidyverse)

Pn_identity100.distr <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/NEWDATA/18S_V4_1191_samples.results/18S_V4_1191_Pn_samples_3readsmin_pid100.distr")

Pn_identity100.tax <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/NEWDATA/18S_V4_1191_samples.results/18S_V4_1191_Pn_samples_3readsmin_pid100.results", header = F, sep = "\t")

Pn_identity100.distr.tax <- inner_join(Pn_identity100.distr, Pn_identity100.tax, by = "V1") %>%
  select(-V2.y)

colnames(Pn_identity100.distr.tax) <- c("seqId", "sample", "readcounts", "perc.identity", "lineage", "GB_ID")


env <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/18S_V4_1191_samples_context.tsv")

Pn_identity100.distr.tax.env <- inner_join(Pn_identity100.distr.tax, env, by = "sample")
...
Pn_identity100.distr.tax.env1 <- splitstackshape::cSplit(Pn_identity100.distr.tax.env, "lineage", "|") %>%
  ...
  group_by(seqId) %>% 
  ungroup() %>% 
  select(seqId, sample, readcounts, perc.identity:GB_ID, species, Station.label:Depth.chloro.max)
...
The two datasets are quite different in the number of amplicons. **TARA-PR2** has **7 sequences**, while **TARA-MC** has **324**. There are **4 shared sequences** between the two. In terms of species, they **share 1 specie**, with TARA-PR2 containing 3 species never detected in TARA-MC, and the latter with 22 species never annotated in TARA-PR2. [**]
...
The **total number of reads** (plot on the left) is higher than 100000 in both datasets, with TARA-MC containing around 300000 reads more than TARA-PR2.
...
The **richness** - i.e. number of species detected - (bottom plot) follows similar trends between the two datasets along the sampling stations, but with many exceptions. 
This trend is more evident when looking at the total abundance per station - expressed as total number of read counts per stations - (top plot). Some exceptions are the cases of stations 006, 153, 158 and 210 where TARA-MC exceeds TARA-PR2 by thousands of units.
...
There is a significant correlation between richness and abundance in both dataset, slightly weaker in TARA-PR2 (rho = 0.52) than in TARA-MC (rho = 0.55).
...
The plots show thata **there is spatial autocorrelation**. 

In particular, in TARA-PR2 graph (left) there is one spike at Lag = 1, Lag = 3 and Lag = 4 that clearly falls outside the bonds, meaning that the total abundance in each station consistently influences the abundance in the following station, and of the 3rd and the 4th.

In TARA-MC (right graph) there is a peak at 14 that slightly exceeds the outside bonds.
...
**The two taxonomies correspond**, except for one sequence that is assigned as *P. seriata* according to TARA-PR2, while for TARA_MC it is ascribed to the group of P. arenisensis, P. dolorosa, P. arctica and P. subcurvata.
  ...
  geom_scatterpie(aes(x=Long, y=Lat, r=radius),
                ...
                #          "australis", "micropora", "delicatissima",
                 #         "galaxiae", "brasiliana", "others (*)"),
                  #alpha=.7) +
                cols = c("cuspidata", "decipiens","delicatissima",
                         ...
                         "sabit", "seriata", "turgidula", "unknown"),
                         alpha = .7) +
  theme_minimal() +
  theme(legend.text=element_text(size=10)) +
  coord_fixed() +
