...
awk '$3>99.99' 18S_V4_1191_samples_Pn.results > 18S_V4_1191_samples_Pn_identity100.results

#they are 17

#Extract only seq ids (column 1)
awk '{print $1}' 18S_V4_1191_samples_Pn_identity100.results > Pn_identity100_seqid.txt

#use the retrieved 17 sequence Ids to subset the distr file (i.e. the one 
#containing seqId|sampleId|readcounts [*]

srun grep -Ff Pn_identity100_seqid.txt 18S_V4_1191_samples_3readsmin.distr > 18S_V4_1191_samples_3readsmin_Pn_identity100.distr &

#[*]note: I am using a distr files that contains only sequences that have a minimum number of reads equal to 3, as adviced by Nicholas Henry.

#the final output is made by 14 different sequences

```

There are 17 different sequences assigned to Pseudonitzschia species with a **100% identity** to PR2 database. **14** out of these 17 are also occurring at least **3 times** in the dataset. 

<br>

3. Add the environmental information for the selected 14 sequences

[note: Still waiting the information about the length of the sequences.]
...
Pn_identity100.distr <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/18S_V4_1191_samples_3readsmin_Pn_identity100.distr") #14 sequences

Pn_identity100.tax <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/18S_V4_1191_samples_Pn_identity100.results", header = F, sep = "\t") #17 sequences


Pn_identity100.distr.tax <- inner_join(Pn_identity100.distr, Pn_identity100.tax, by = "V1") %>% 
  select(-V2.y)

colnames(Pn_identity100.distr.tax) <- c("seqId", "sample", "readcounts", "perc.identity", "lineage", "idpr2")


env <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/18S_V4_1191_samples_context.tsv")

Pn_identity100.distr.tax.env <- inner_join(Pn_identity100.distr.tax, env)
...
Pn_identity100.distr.tax.env1 <- splitstackshape::cSplit(Pn_identity100.distr.tax.env, "lineage", "|") %>% 
  ...
  group_by(seqId) %>% 
  ungroup() %>% 
  select(seqId:readcounts, perc.identity:idpr2, species, Station.label:Depth.chloro.max)


final_tara_pr2<- Pn_identity100.distr.tax.env1 %>% 
  select(seqId, species, sample, readcounts, Station.label:Depth.chloro.max) %>%
  mutate(dataset = c("TARA-PR2"))


#final_taraPR2 is the final file with seqIds, taxonomical annotation, distribution and environmental info. 
```
...
#subset the distr file using my seq Ids

srun grep -Ff Pn_tara_candi_final_pid100_list_clean.txt ~/Data/metaB/V4/18S_V4_1191_samples_3readsmin.distr > 18S_V4_1191_samples_3readsmin_Pn_candi_identity100.distr &

```

There are 508 different sequences assigned to Pseudonitzschia species with a **100% identity** to MareChiara database. **210** out of these 508 are also occurring at least **3 times** in the dataset. 

<br> 

2. Add the environmental information for the selected 210 sequences

```{r warning=FALSE, message=FALSE, cache=TRUE}

Pn_candi_identity100.distr <- fread("C:/Users/Userszn/Documents/PhD/script/Pseudonitzschia/metaB/V4/cfr100pid/18S_V4_1191_samples_3readsmin_Pn_candi_identity100.distr") 
...
Pn_candi_identity100.distr.tax <- inner_join(Pn_candi_identity100.tax, Pn_candi_identity100.distr)
...
final_tara_mc <- Pn_candi_identity100.distr.tax.env %>% 
  mutate(species = case_when(grepl("allochrona", tax) ~ "allochrona",
                             grepl("americana", tax) ~ "americana",
                             grepl("arenysensis", tax) ~ "arenysensis",
                             grepl("australis", tax) ~ "australis",
                             grepl("batesiana", tax) ~ "batesiana",
                             grepl("brasiliana", tax) ~ "brasiliana",
                             ...
                             grepl("kodamae", tax) ~ "kodamae",
                             grepl("mannii", tax) ~ "mannii",
                             grepl("micropora", tax) ~ "micropora",
                             grepl("multiseries", tax) ~ "multiseries",
...
The two datasets are quite different in the number of amplicons. **TARA-PR2** has **14 sequences**, while **TARA-MC** has **508**. There are **8 shared sequences** between the two. In terms of species, they share 4 species, with TARA-PR2 containing 3 species never detected in TARA-MC, and the latter with 17 species never annotated in TARA-PR2. 
...
The **richness** - i.e. number of species detected - (upper plot) follows the same trend between the two datasets along the sampling stations, with some exceptions (i.e. stations 18-25, 114-115, 180-188). 
...
There is a significant correlation between richness and abundance in both dataset, weak in TARA-PR2 (rho = 0.3) and stronger in TARA-MC (rho = 0.5).
...
The plots show thata **there is spatial autocorrelation in TARA-PR2, but not in TARA-MC**. 

- In particular, in TARA-PR2 there are two spikes that fall outside the bonds (one at 1 and one at 5), meaning that the total abundance in each station consistently influences the abundance in the following station and the 5th station after.

- In contrast, in the TARA-MC graph the only value outside the bonds is the spike at lag 0, that indicates the correlation of a sample with itself. The abundance per station values are thus here independent of each other if only trying to explain them with themselves (the autocorrelation property).
...
Faccio un confronto tra le distribuzioni mie e le loro al 100% per rispondere alla domanda "Qual e' l'impatto di avere un data reference fatto bene sull'assegnazione tassonomica?"):
-	Statistiche e mappe
Le specie sono le stesse?
La distribuzione e' la stessa?
-	Albero1 che include le reference + nostre 100% + Tara assegnate 100% (**)